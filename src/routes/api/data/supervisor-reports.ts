...
export const Route = createFileRoute("/api/data/supervisor-reports")({
  server: {
    handlers: {
      GET: async () => {
        try {
          ...
          const rows = await loadShared("supervisor_reports");
      ...
      POST: async ({ request }) => {
        try {
          ...
          await syncShared("supervisor_reports", "created_by", user.id, rows);
