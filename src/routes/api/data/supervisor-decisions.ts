...
export const Route = createFileRoute("/api/data/supervisor-decisions")({
  server: {
    handlers: {
      GET: async () => {
        try {
          ...
          const rows = await loadShared("supervisor_decisions");
      ...
      POST: async ({ request }) => {
        try {
          ...
          await syncShared("supervisor_decisions", "created_by", user.id, rows);
