...
export const Route = createFileRoute("/api/data/translations")({
  server: {
    handlers: {
      GET: async () => {
        try {
          ...
          const rows = await loadShared("translations");
      ...
      POST: async ({ request }) => {
        try {
          ...
          await syncShared("translations", "user_id", user.id, rows, ["key_points", "doc_meta"]);
