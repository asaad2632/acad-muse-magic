...
export const Route = createFileRoute("/api/data/custom-formats")({
  server: {
    handlers: {
      GET: async () => {
        try {
          ...
          const rows = await loadShared("custom_formats");
      ...
      POST: async ({ request }) => {
        try {
          ...
          await syncShared("custom_formats", "user_id", user.id, rows, ["templates"]);
