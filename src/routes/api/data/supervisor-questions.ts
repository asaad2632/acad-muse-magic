...
export const Route = createFileRoute("/api/data/supervisor-questions")({
  server: {
    handlers: {
      GET: async () => {
        try {
          ...
          const rows = await loadShared("supervisor_questions");
      ...
      POST: async ({ request }) => {
        try {
          ...
          await syncShared("supervisor_questions", "created_by", user.id, rows);
